...
C
      ...
      TI = AMIN1(T(I),273.)
      FR = (TI - 203.)/70.
c     print *, 'so31 =', so31 they are the same 
c      print *, 'FR =', FR  they are the same 
  29  SO3(I) = FR*SO32(L) + (1.-FR)*SO31(L)
c      print *, ' PO3D(1)',PO3D(1)

C here, so3 are the same across two code 
      GO TO 64
C
...
c      if(L.eq.55)print *,'In PHOTO, AT, BT,CT,WAV,ALMO,RO3,T'
      DO 54 I=1,NZ
